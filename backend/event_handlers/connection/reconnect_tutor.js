...
const handler = async (context, user_id, session_code) => {
  ...
  try {
    ...
    // If user is a tutor in the room
    if (session.tutors.some(tutor => tutor._id.toString() === user_id)) {
      ...
      console.info(`RECONNECT_TUTOR: Tutor ${user_id} reconnected to session ${session_code}`);
    ...
    console.warn(`RECONNECT_TUTOR: A tutor failed to reconnect to session ${session_code}`, err);
    return {success: false, msg: `Unable to start session ${session_code}`};
  }
}
